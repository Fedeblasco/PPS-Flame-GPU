 ...
 *
 */
__global__ void GPUFLAME_nucleate(xmachine_memory_crystal_list* agents, xmachine_message_internal_coord_list* internal_coord_messages){
	...
	//SoA to AoS - xmachine_memory_nucleate Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_crystal agent;
	...
	//FLAME function call
	int dead = !nucleate(&agent, internal_coord_messages);
	...
	//AoS to SoA - xmachine_memory_nucleate Coalesced memory write (ignore arrays)
	agents->rank[index] = agent.rank;
	agents->l[index] = agent.l;
	agents->bin[index] = agent.bin;
	}
 ...
 *
 */
__global__ void GPUFLAME_growth(xmachine_memory_crystal_list* agents){
	...
	//SoA to AoS - xmachine_memory_growth Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_crystal agent;
	...
	//FLAME function call
	int dead = !growth(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_growth Coalesced memory write (ignore arrays)
	agents->rank[index] = agent.rank;
	agents->l[index] = agent.l;
	agents->bin[index] = agent.bin;
}
