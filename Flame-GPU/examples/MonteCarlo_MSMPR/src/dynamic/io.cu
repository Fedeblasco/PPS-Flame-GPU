...
void saveIterationData(char* outputpath, int iteration_number, xmachine_memory_crystal_list* h_crystals_default, xmachine_memory_crystal_list* d_crystals_default, int h_xmachine_memory_crystal_default_count)
{
    ...
    fputs("\t<DELTA_T>", file);
    sprintf(data, "%f", (*get_DELTA_T()));
    fputs(data, file);
    fputs("</DELTA_T>\n", file);
    fputs("\t<B0>", file);
    sprintf(data, "%f", (*get_B0()));
    fputs(data, file);
    fputs("</B0>\n", file);
    fputs("\t<G0>", file);
    ...
    fputs("</G0>\n", file);
    fputs("\t<b>", file);
    sprintf(data, "%f", (*get_b()));
    fputs(data, file);
    fputs("</b>\n", file);
    fputs("\t<y>", file);
    sprintf(data, "%f", (*get_y()));
    fputs(data, file);
    fputs("</y>\n", file);
    fputs("\t<t>", file);
    sprintf(data, "%d", (*get_t()));
    fputs(data, file);
    fputs("</t>\n", file);
    fputs("\t<m0>", file);
    sprintf(data, "%f", (*get_m0()));
    fputs(data, file);
    fputs("</m0>\n", file);
    fputs("\t<delta_m0>", file);
    sprintf(data, "%f", (*get_delta_m0()));
    fputs(data, file);
    fputs("</delta_m0>\n", file);
    fputs("\t<nuc_no>", file);
    sprintf(data, "%d", (*get_nuc_no()));
    fputs(data, file);
    fputs("</nuc_no>\n", file);
    fputs("\t<exit_no>", file);
    sprintf(data, "%d", (*get_exit_no()));
    fputs(data, file);
    fputs("</exit_no>\n", file);
    fputs("\t<agg_no>", file);
    sprintf(data, "%d", (*get_agg_no()));
    fputs(data, file);
    fputs("</agg_no>\n", file);
	fputs("</environment>\n" , file);
	...
	*h_xmachine_memory_crystal_count = 0;
	...
	while(reading==1)
	{
		...
		else if(c == '>')
		{
            ...
            if(strcmp(buffer, "DELTA_T") == 0) in_env_DELTA_T = 1;
            if(strcmp(buffer, "/DELTA_T") == 0) in_env_DELTA_T = 0;
			if(strcmp(buffer, "B0") == 0) in_env_B0 = 1;
            if(strcmp(buffer, "/B0") == 0) in_env_B0 = 0;
			if(strcmp(buffer, "G0") == 0) in_env_G0 = 1;
            if(strcmp(buffer, "/G0") == 0) in_env_G0 = 0;
			if(strcmp(buffer, "b") == 0) in_env_b = 1;
            if(strcmp(buffer, "/b") == 0) in_env_b = 0;
			if(strcmp(buffer, "y") == 0) in_env_y = 1;
            if(strcmp(buffer, "/y") == 0) in_env_y = 0;
			if(strcmp(buffer, "t") == 0) in_env_t = 1;
            if(strcmp(buffer, "/t") == 0) in_env_t = 0;
			if(strcmp(buffer, "m0") == 0) in_env_m0 = 1;
            if(strcmp(buffer, "/m0") == 0) in_env_m0 = 0;
			if(strcmp(buffer, "delta_m0") == 0) in_env_delta_m0 = 1;
            if(strcmp(buffer, "/delta_m0") == 0) in_env_delta_m0 = 0;
			if(strcmp(buffer, "nuc_no") == 0) in_env_nuc_no = 1;
            if(strcmp(buffer, "/nuc_no") == 0) in_env_nuc_no = 0;
			if(strcmp(buffer, "exit_no") == 0) in_env_exit_no = 1;
            if(strcmp(buffer, "/exit_no") == 0) in_env_exit_no = 0;
			if(strcmp(buffer, "agg_no") == 0) in_env_agg_no = 1;
            if(strcmp(buffer, "/agg_no") == 0) in_env_agg_no = 0;
		...
		else if(c == '<')
		{
            ...
            if(in_env_DELTA_T){
                    ...
                    env_DELTA_T = (float) fgpu_atof(buffer);
                    
                    set_DELTA_T(&env_DELTA_T);
                  
              }
            if(in_env_B0){
              
                    env_B0 = (float) fgpu_atof(buffer);
            ...
            if(in_env_delta_m0){
                    ...
                    env_delta_m0 = (float) fgpu_atof(buffer);
                    
                    set_delta_m0(&env_delta_m0);
                  
              }
            if(in_env_nuc_no){
              
                    env_nuc_no = (int) fpgu_strtol(buffer);
                    
                    set_nuc_no(&env_nuc_no);
                  
              }
            if(in_env_exit_no){
              
                    env_exit_no = (int) fpgu_strtol(buffer);
                    
                    set_exit_no(&env_exit_no);
                  
              }
            if(in_env_agg_no){
              
                    env_agg_no = (int) fpgu_strtol(buffer);
                    
                    set_agg_no(&env_agg_no);
