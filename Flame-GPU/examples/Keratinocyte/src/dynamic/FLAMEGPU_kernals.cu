...
//Spatial Partitioning Variables
__constant__ glm::vec3 d_message_force_min_bounds;           /**< min bounds (x,y,z) of partitioning environment */
__constant__ glm::vec3 d_message_force_max_bounds;           /**< max bounds (x,y,z) of partitioning environment */
__constant__ glm::ivec3 d_message_force_partitionDim;           /**< partition dimensions (x,y,z) of partitioning environment */
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created keratinocyte agent functions */
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created force message functions */
 ...
 * @param position glm::vec3 vector representing a position
 */
__device__ glm::ivec3 message_force_grid_position(glm::vec3 position)
{
    glm::ivec3 gridPos;
 ...
 * @param gridPos The position in partition space
 */
__device__ unsigned int message_force_hash(glm::ivec3 gridPos)
{
	//cheap bounding without mod (within range +- partition dimension)
	...
	__global__ void hist_force_messages(uint* local_bin_index, uint* unsorted_index, int* global_bin_count, xmachine_message_force_list* messages, int agent_count)
	{
		...
		if (index >= agent_count)
			return;
        glm::vec3 position = glm::vec3(messages->x[index], messages->y[index], messages->z[index]);
		glm::ivec3 grid_position = message_force_grid_position(position);
 ...
 * @return true if a message has been loaded into sm false otherwise
 */
__device__ bool load_next_force_message(xmachine_message_force_list* messages, xmachine_message_force_PBM* partition_matrix, glm::ivec3 relative_cell, int cell_index_max, glm::ivec3 agent_grid_cell, int cell_index)
{
	extern __shared__ int sm_data [];
	char* message_share = (char*)&sm_data[0];
...
__device__ xmachine_message_force* get_first_force_message(xmachine_message_force_list* messages, xmachine_message_force_PBM* partition_matrix, float x, float y, float z){
	...
	int cell_index = 0;
	glm::vec3 position = glm::vec3(x, y, z);
	glm::ivec3 agent_grid_cell = message_force_grid_position(position);
