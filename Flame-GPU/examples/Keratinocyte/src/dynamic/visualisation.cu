...
cudaGraphicsResource_t keratinocyte_default_cgr;
GLuint keratinocyte_default_tbo;
GLuint keratinocyte_default_displacementTex;

cudaGraphicsResource_t keratinocyte_resolve_cgr;
...
__global__ void output_keratinocyte_agent_to_VBO(xmachine_memory_keratinocyte_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&keratinocyte_default_cgr, &keratinocyte_default_tbo, &keratinocyte_default_displacementTex, xmachine_memory_keratinocyte_MAX * sizeof( glm::vec4));
	
	createTBO(&keratinocyte_resolve_cgr, &keratinocyte_resolve_tbo, &keratinocyte_resolve_displacementTex, xmachine_memory_keratinocyte_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_keratinocyte_default_count() > 0)
	{
        ...
        gpuErrchk(cudaGraphicsMapResources(1, &keratinocyte_default_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, keratinocyte_default_cgr));
		//cuda block size
		...
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &keratinocyte_default_cgr));
	}
	...
	if (get_agent_keratinocyte_resolve_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &keratinocyte_resolve_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, keratinocyte_resolve_cgr));
		//cuda block size
		...
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &keratinocyte_resolve_cgr));
	}
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case(27) :
		...
		deleteTBO( &keratinocyte_default_cgr, &keratinocyte_default_tbo);
		
		deleteTBO( &keratinocyte_resolve_cgr, &keratinocyte_resolve_tbo);
