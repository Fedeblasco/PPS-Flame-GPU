...
void cleanup(){
    ...
    if(d_temp_scan_storage_cell != nullptr){
...
PROFILE_SCOPED_RANGE("singleIteration");
  ...
  /* set all non partitioned, spatial partitioned and On-Graph Partitioned message counts to 0*/
    ...
    PROFILE_PUSH_RANGE("cell_output_state");
    ...
    PROFILE_PUSH_RANGE("cell_update_state");
...
int min_cell_default_state_variable(){
    //min in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_cells_default->state);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_cell_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
int max_cell_default_state_variable(){
    //max in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_cells_default->state);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_cell_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
int min_cell_default_x_variable(){
    //min in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_cells_default->x);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_cell_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
int max_cell_default_x_variable(){
    //max in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_cells_default->x);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_cell_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
...
int min_cell_default_y_variable(){
    //min in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_cells_default->y);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_cell_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
int max_cell_default_y_variable(){
    //max in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_cells_default->y);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_cell_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
