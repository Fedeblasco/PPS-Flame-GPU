...
cudaGraphicsResource_t cell_default_cgr;
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&cell_default_cgr, &cell_default_tbo, &cell_default_displacementTex, xmachine_memory_cell_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_cell_default_count() > 0)
	{
        ...
        gpuErrchk(cudaGraphicsMapResources(1, &cell_default_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, cell_default_cgr));
		//cuda block size
		...
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &cell_default_cgr));
	}
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case(27) :
		...
		deleteTBO( &cell_default_cgr, &cell_default_tbo);
