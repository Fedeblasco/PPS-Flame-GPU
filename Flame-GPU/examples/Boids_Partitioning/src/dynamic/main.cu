...
void runConsoleWithXMLOutput(int iterations, int outputFrequency){
	...
	for (int i=0; i< iterations; i++)
	{
		...
		if((i+1) % outputFrequency == 0){
			saveIterationData(outputpath, i+1, get_host_Boid_default_agents(), get_device_Boid_default_agents(), get_agent_Boid_default_count());
	...
	if(iterations % outputFrequency != 0){
		saveIterationData(outputpath, iterations, get_host_Boid_default_agents(), get_device_Boid_default_agents(), get_agent_Boid_default_count());
		printf("Iteration %i Saved to XML\n", iterations);
	}
