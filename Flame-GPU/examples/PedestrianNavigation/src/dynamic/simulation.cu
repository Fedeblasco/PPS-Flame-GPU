...
xmachine_message_specialist_terminated_list* h_specialist_terminateds;         /**< Pointer to message list on host*/
xmachine_message_specialist_terminated_list* d_specialist_terminateds;         /**< Pointer to message list on device*/
xmachine_message_specialist_terminated_list* d_specialist_terminateds_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_specialist_terminated_count;         /**< message list counter*/
int h_message_specialist_terminated_output_type;   /**< message output type (single or optional)*/

/* free_specialist Message variables */
xmachine_message_free_specialist_list* h_free_specialists;         /**< Pointer to message list on host*/
xmachine_message_free_specialist_list* d_free_specialists;         /**< Pointer to message list on device*/
xmachine_message_free_specialist_list* d_free_specialists_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_free_specialist_count;         /**< message list counter*/
int h_message_free_specialist_output_type;   /**< message output type (single or optional)*/
...
xmachine_message_free_box_list* h_free_boxs;         /**< Pointer to message list on host*/
xmachine_message_free_box_list* d_free_boxs;         /**< Pointer to message list on device*/
xmachine_message_free_box_list* d_free_boxs_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_free_box_count;         /**< message list counter*/
int h_message_free_box_output_type;   /**< message output type (single or optional)*/
...
/** agent_receive_specialist_terminated
 * Agent function prototype for receive_specialist_terminated function of agent agent
 */
void agent_receive_specialist_terminated(cudaStream_t &stream);
...
/** specialist_manager_receive_free_specialist
 * Agent function prototype for receive_free_specialist function of specialist_manager agent
 */
void specialist_manager_receive_free_specialist(cudaStream_t &stream);
...
/** triage_receive_free_box
 * Agent function prototype for receive_free_box function of triage agent
 */
void triage_receive_free_box(cudaStream_t &stream);
...
void initialise(char * inputfile){
	...
	int message_specialist_terminated_SoA_size = sizeof(xmachine_message_specialist_terminated_list);
	h_specialist_terminateds = (xmachine_message_specialist_terminated_list*)malloc(message_specialist_terminated_SoA_size);
	int message_free_specialist_SoA_size = sizeof(xmachine_message_free_specialist_list);
	...
	int message_free_box_SoA_size = sizeof(xmachine_message_free_box_list);
	h_free_boxs = (xmachine_message_free_box_list*)malloc(message_free_box_SoA_size);
  ...
  gpuErrchk( cudaMalloc( (void**) &d_xmachine_memory_triage_keys, xmachine_memory_triage_MAX* sizeof(uint)));
	...
	/* specialist_terminated Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_specialist_terminateds, message_specialist_terminated_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_specialist_terminateds_swap, message_specialist_terminated_SoA_size));
	gpuErrchk( cudaMemcpy( d_specialist_terminateds, h_specialist_terminateds, message_specialist_terminated_SoA_size, cudaMemcpyHostToDevice));
	
	/* free_specialist Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_free_specialists, message_free_specialist_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_free_specialists_swap, message_free_specialist_SoA_size));
	gpuErrchk( cudaMemcpy( d_free_specialists, h_free_specialists, message_free_specialist_SoA_size, cudaMemcpyHostToDevice));
	...
	/* free_box Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_free_boxs, message_free_box_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_free_boxs_swap, message_free_box_SoA_size));
	gpuErrchk( cudaMemcpy( d_free_boxs, h_free_boxs, message_free_box_SoA_size, cudaMemcpyHostToDevice));
...
void cleanup(){
  ...
  temp_scan_bytes_xmachine_message_pedestrian_state = 0;
	...
	/* specialist_terminated Message variables */
	free( h_specialist_terminateds);
	gpuErrchk(cudaFree(d_specialist_terminateds));
	gpuErrchk(cudaFree(d_specialist_terminateds_swap));
	
	/* free_specialist Message variables */
	free( h_free_specialists);
	gpuErrchk(cudaFree(d_free_specialists));
	gpuErrchk(cudaFree(d_free_specialists_swap));
	...
	/* free_box Message variables */
	free( h_free_boxs);
	gpuErrchk(cudaFree(d_free_boxs));
	gpuErrchk(cudaFree(d_free_boxs_swap));
...
PROFILE_SCOPED_RANGE("singleIteration");
  ...
  /* set all non partitioned, spatial partitioned and On-Graph Partitioned message counts to 0*/
	...
	h_message_specialist_terminated_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_specialist_terminated_count, &h_message_specialist_terminated_count, sizeof(int)));
	
	h_message_free_specialist_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_free_specialist_count, &h_message_free_specialist_count, sizeof(int)));
	...
	h_message_free_box_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_free_box_count, &h_message_free_box_count, sizeof(int)));
	...
	specialist_receive_specialist_reached(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
    ...
    PROFILE_PUSH_RANGE("agent_receive_specialist_terminated");
	agent_receive_specialist_terminated(stream3);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: agent_receive_specialist_terminated = %f (ms)\n", instrument_milliseconds);
#endif
	...
	agent_receive_doctor_response(stream5);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	...
	agent_receive_specialist_response(stream6);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	...
	doctor_manager_receive_free_doctors(stream7);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	...
	agent_receive_check_in_response(stream3);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	...
	chair_admin_attend_chair_petitions(stream4);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
    ...
    PROFILE_PUSH_RANGE("specialist_manager_receive_free_specialist");
	specialist_manager_receive_free_specialist(stream3);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: specialist_manager_receive_free_specialist = %f (ms)\n", instrument_milliseconds);
#endif
    ...
    PROFILE_PUSH_RANGE("triage_receive_free_box");
	triage_receive_free_box(stream4);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: triage_receive_free_box = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
...
//constant setter
void set_EXIT_Y(int* h_EXIT_Y){
    gpuErrchk(cudaMemcpyToSymbol(EXIT_Y, h_EXIT_Y, sizeof(int)));
    memcpy(&h_env_EXIT_Y, h_EXIT_Y,sizeof(int));
}
...
int min_agent_default_id_variable(){
    ...
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_agents_default->id);
...
void agent_receive_box_response(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_free_box_count + h_xmachine_memory_agent_count > xmachine_message_free_box_MAX){
		printf("Error: Buffer size of free_box message will be exceeded in function receive_box_response\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_free_box_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_free_box_output_type, &h_message_free_box_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_free_box_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_free_box_swaps<<<gridSize, blockSize, 0, stream>>>(d_free_boxs); 
	gpuErrchkLaunch();
	...
	//Output       : free_box
	//Agent Output : 
	GPUFLAME_receive_box_response<<<g, b, sm_size, stream>>>(d_agents, d_box_responses, d_free_boxs);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//free_box Message Type Prefix Sum
	
	//swap output
	xmachine_message_free_box_list* d_free_boxs_scanswap_temp = d_free_boxs;
	d_free_boxs = d_free_boxs_swap;
	d_free_boxs_swap = d_free_boxs_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_agent, 
        temp_scan_storage_bytes_agent, 
        d_free_boxs_swap->_scan_input,
        d_free_boxs_swap->_position,
        h_xmachine_memory_agent_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_free_box_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_free_box_messages<<<gridSize, blockSize, 0, stream>>>(d_free_boxs, d_free_boxs_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_free_boxs_swap->_position[h_xmachine_memory_agent_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_free_boxs_swap->_scan_input[h_xmachine_memory_agent_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_free_box_count += scan_last_sum+1;
	}else{
		h_message_free_box_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_free_box_count, &h_message_free_box_count, sizeof(int)));	
...
/* Shared memory size calculator for agent function */
int agent_receive_specialist_terminated_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_specialist_terminated));
...
void agent_receive_specialist_terminated(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, receive_specialist_terminated_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	receive_specialist_terminated_function_filter<<<gridSize, blockSize, 0, stream>>>(d_agents_default, d_agents);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_free_specialist_count + h_xmachine_memory_agent_count > xmachine_message_free_specialist_MAX){
		printf("Error: Buffer size of free_specialist message will be exceeded in function receive_specialist_terminated\n");
		exit(EXIT_FAILURE);
	}
	
	
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_receive_specialist_terminated, agent_receive_specialist_terminated_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = agent_receive_specialist_terminated_sm_size(blockSize);
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_free_specialist_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_free_specialist_output_type, &h_message_free_specialist_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_free_specialist_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_free_specialist_swaps<<<gridSize, blockSize, 0, stream>>>(d_free_specialists); 
	gpuErrchkLaunch();
	
	
	//MAIN XMACHINE FUNCTION CALL (receive_specialist_terminated)
	//Reallocate   : false
	//Input        : specialist_terminated
	//Output       : free_specialist
	//Agent Output : 
	GPUFLAME_receive_specialist_terminated<<<g, b, sm_size, stream>>>(d_agents, d_specialist_terminateds, d_free_specialists);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//free_specialist Message Type Prefix Sum
	
	//swap output
	xmachine_message_free_specialist_list* d_free_specialists_scanswap_temp = d_free_specialists;
	d_free_specialists = d_free_specialists_swap;
	d_free_specialists_swap = d_free_specialists_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_agent, 
        temp_scan_storage_bytes_agent, 
        d_free_specialists_swap->_scan_input,
        d_free_specialists_swap->_position,
        h_xmachine_memory_agent_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_free_specialist_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_free_specialist_messages<<<gridSize, blockSize, 0, stream>>>(d_free_specialists, d_free_specialists_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_free_specialists_swap->_position[h_xmachine_memory_agent_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_free_specialists_swap->_scan_input[h_xmachine_memory_agent_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_free_specialist_count += scan_last_sum+1;
	}else{
		h_message_free_specialist_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_free_specialist_count, &h_message_free_specialist_count, sizeof(int)));	
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_agent_default_count+h_xmachine_memory_agent_count > xmachine_memory_agent_MAX){
		printf("Error: Buffer size of receive_specialist_terminated agents in state default will be exceeded moving working agents to next state in function receive_specialist_terminated\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int specialist_manager_receive_free_specialist_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_free_specialist));
...
void specialist_manager_receive_free_specialist(cudaStream_t &stream){
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_receive_free_specialist, specialist_manager_receive_free_specialist_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = specialist_manager_receive_free_specialist_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (receive_free_specialist)
	//Reallocate   : false
	//Input        : free_specialist
	//Output       : 
	//Agent Output : 
	GPUFLAME_receive_free_specialist<<<g, b, sm_size, stream>>>(d_specialist_managers, d_free_specialists);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_specialist_manager_defaultSpecialistManager_count+h_xmachine_memory_specialist_manager_count > xmachine_memory_specialist_manager_MAX){
		printf("Error: Buffer size of receive_free_specialist agents in state defaultSpecialistManager will be exceeded moving working agents to next state in function receive_free_specialist\n");
      exit(EXIT_FAILURE);
      }
...
void specialist_receive_specialist_reached(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_specialist_terminated_count + h_xmachine_memory_specialist_count > xmachine_message_specialist_terminated_MAX){
		printf("Error: Buffer size of specialist_terminated message will be exceeded in function receive_specialist_reached\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_specialist_terminated_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_specialist_terminated_output_type, &h_message_specialist_terminated_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_specialist_terminated_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_specialist_terminated_swaps<<<gridSize, blockSize, 0, stream>>>(d_specialist_terminateds); 
	gpuErrchkLaunch();
	...
	//Output       : specialist_terminated
	//Agent Output : 
	GPUFLAME_receive_specialist_reached<<<g, b, sm_size, stream>>>(d_specialists, d_specialist_reacheds, d_specialist_terminateds);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//specialist_terminated Message Type Prefix Sum
	
	//swap output
	xmachine_message_specialist_terminated_list* d_specialist_terminateds_scanswap_temp = d_specialist_terminateds;
	d_specialist_terminateds = d_specialist_terminateds_swap;
	d_specialist_terminateds_swap = d_specialist_terminateds_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_specialist, 
        temp_scan_storage_bytes_specialist, 
        d_specialist_terminateds_swap->_scan_input,
        d_specialist_terminateds_swap->_position,
        h_xmachine_memory_specialist_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_specialist_terminated_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_specialist_terminated_messages<<<gridSize, blockSize, 0, stream>>>(d_specialist_terminateds, d_specialist_terminateds_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_specialist_terminateds_swap->_position[h_xmachine_memory_specialist_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_specialist_terminateds_swap->_scan_input[h_xmachine_memory_specialist_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_specialist_terminated_count += scan_last_sum+1;
	}else{
		h_message_specialist_terminated_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_specialist_terminated_count, &h_message_specialist_terminated_count, sizeof(int)));	
...
void agent_generator_generate_personal(cudaStream_t &stream){
	...
	//reset agent count
	int agent_after_birth_count;
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_agents_new->_position[agent_generators_pre_death_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_agents_new->_scan_input[agent_generators_pre_death_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	if (scan_last_included == 1)
		agent_after_birth_count = h_xmachine_memory_agent_default_count + scan_last_sum+1;
	else
		agent_after_birth_count = h_xmachine_memory_agent_default_count + scan_last_sum;
	...
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_agent_Agents, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_agent_Agents<<<gridSize, blockSize, 0, stream>>>(d_agents_default, d_agents_new, h_xmachine_memory_agent_default_count, agent_generators_pre_death_count);
	gpuErrchkLaunch();
	//Copy count to device
	h_xmachine_memory_agent_default_count = agent_after_birth_count;
	gpuErrchk( cudaMemcpyToSymbol( d_xmachine_memory_agent_default_count, &h_xmachine_memory_agent_default_count, sizeof(int)));	
...
void box_box_server(cudaStream_t &stream){
	...
	//THERE IS NOT A FUNCTION CONDITION
	//currentState maps to working list
	xmachine_memory_box_list* boxs_defaultBox_temp = d_boxs;
	d_boxs = d_boxs_defaultBox;
	d_boxs_defaultBox = boxs_defaultBox_temp;
	//set working count to current state count
	...
	if (h_message_box_response_count + h_xmachine_memory_box_count > xmachine_message_box_response_MAX){
		printf("Error: Buffer size of box_response message will be exceeded in function box_server\n");
		exit(EXIT_FAILURE);
	}
	...
	//Output       : box_response
	//Agent Output : 
	GPUFLAME_box_server<<<g, b, sm_size, stream>>>(d_boxs, d_box_petitions, d_box_responses, d_rand48);
	gpuErrchkLaunch();
...
/* Shared memory size calculator for agent function */
int triage_receive_free_box_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_free_box));
...
void triage_receive_free_box(cudaStream_t &stream){
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_receive_free_box, triage_receive_free_box_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = triage_receive_free_box_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (receive_free_box)
	//Reallocate   : false
	//Input        : free_box
	//Output       : 
	//Agent Output : 
	GPUFLAME_receive_free_box<<<g, b, sm_size, stream>>>(d_triages, d_free_boxs);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_triage_defaultTriage_count+h_xmachine_memory_triage_count > xmachine_memory_triage_MAX){
		printf("Error: Buffer size of receive_free_box agents in state defaultTriage will be exceeded moving working agents to next state in function receive_free_box\n");
      exit(EXIT_FAILURE);
      }
