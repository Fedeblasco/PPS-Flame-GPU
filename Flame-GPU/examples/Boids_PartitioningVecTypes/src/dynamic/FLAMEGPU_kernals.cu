...
__constant__ glm::vec3 d_message_location_min_bounds;           /**< min bounds (x,y,z) of partitioning environment */
__constant__ glm::vec3 d_message_location_max_bounds;           /**< max bounds (x,y,z) of partitioning environment */
__constant__ glm::ivec3 d_message_location_partitionDim;           /**< partition dimensions (x,y,z) of partitioning environment */
...
__constant__ int d_tex_xmachine_message_location_z_offset;texture<float, 1, cudaReadModeElementType> tex_xmachine_message_location_vx;
__constant__ int d_tex_xmachine_message_location_vx_offset;texture<float, 1, cudaReadModeElementType> tex_xmachine_message_location_vy;
__constant__ int d_tex_xmachine_message_location_vy_offset;texture<float, 1, cudaReadModeElementType> tex_xmachine_message_location_vz;
__constant__ int d_tex_xmachine_message_location_vz_offset;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created Boid agent functions */
...
__global__ void scatter_Boid_Agents(xmachine_memory_Boid_list* agents_dst, xmachine_memory_Boid_list* agents_src, int dst_agent_count, int number_to_scatter){
	...
	if ((_scan_input == 1)&&(index < number_to_scatter)){
		...
		agents_dst->velocity[output_index] = agents_src->velocity[index];
	}
}
 ...
 * @param position agent variable of type fvec3
 * @param velocity agent variable of type fvec3
 */
template <int AGENT_TYPE>
__device__ void add_Boid_agent(xmachine_memory_Boid_list* agents, int id, fvec3 position, fvec3 velocity){
	...
	agents->velocity[index] = velocity;

}

//non templated version assumes DISCRETE_2D but works also for CONTINUOUS
__device__ void add_Boid_agent(xmachine_memory_Boid_list* agents, int id, fvec3 position, fvec3 velocity){
    add_Boid_agent<DISCRETE_2D>(agents, id, position, velocity);
}
...
__global__ void reorder_Boid_agents(unsigned int* values, xmachine_memory_Boid_list* unordered_agents, xmachine_memory_Boid_list* ordered_agents)
{
	...
	ordered_agents->velocity[index] = unordered_agents->velocity[old_pos];
}
 ...
 * @param vz agent variable of type float
 */
__device__ void add_location_message(xmachine_message_location_list* messages, int id, float x, float y, float z, float vx, float vy, float vz){
...
__global__ void scatter_optional_location_messages(xmachine_message_location_list* messages, xmachine_message_location_list* messages_swap){
	...
	if (_scan_input == 1){
		...
		messages->vz[output_index] = messages_swap->vz[index];				
	}
}
 ...
 */
__device__ glm::ivec3 message_location_grid_position(glm::vec3 position)
{
    glm::ivec3 gridPos;
 ...
 */
__device__ unsigned int message_location_hash(glm::ivec3 gridPos)
{
	...
	__global__ void hist_location_messages(uint* local_bin_index, uint* unsorted_index, int* global_bin_count, xmachine_message_location_list* messages, int agent_count)
	{
		...
		glm::ivec3 grid_position = message_location_grid_position(position);
	 ...
	 __global__ void reorder_location_messages(uint* local_bin_index, uint* unsorted_index, int* pbm_start_index, xmachine_message_location_list* unordered_messages, xmachine_message_location_list* ordered_messages, int agent_count)
	{
		...
		ordered_messages->vz[sorted_index] = unordered_messages->vz[index];
	}
	...
	__global__ void reorder_location_messages(uint* keys, uint* values, xmachine_message_location_PBM* matrix, xmachine_message_location_list* unordered_messages, xmachine_message_location_list* ordered_messages)
	{
		...
		ordered_messages->vz[index] = unordered_messages->vz[old_pos];
	}
 ...
 */
__device__ bool load_next_location_message(xmachine_message_location_list* messages, xmachine_message_location_PBM* partition_matrix, glm::ivec3 relative_cell, int cell_index_max, glm::ivec3 agent_grid_cell, int cell_index)
{
	extern __shared__ int sm_data [];
	...
	//Using texture cache
  temp_message.id = tex1Dfetch(tex_xmachine_message_location_id, cell_index + d_tex_xmachine_message_location_id_offset); temp_message.x = tex1Dfetch(tex_xmachine_message_location_x, cell_index + d_tex_xmachine_message_location_x_offset); temp_message.y = tex1Dfetch(tex_xmachine_message_location_y, cell_index + d_tex_xmachine_message_location_y_offset); temp_message.z = tex1Dfetch(tex_xmachine_message_location_z, cell_index + d_tex_xmachine_message_location_z_offset); temp_message.vx = tex1Dfetch(tex_xmachine_message_location_vx, cell_index + d_tex_xmachine_message_location_vx_offset); temp_message.vy = tex1Dfetch(tex_xmachine_message_location_vy, cell_index + d_tex_xmachine_message_location_vy_offset); temp_message.vz = tex1Dfetch(tex_xmachine_message_location_vz, cell_index + d_tex_xmachine_message_location_vz_offset); 
...
__device__ xmachine_message_location* get_first_location_message(xmachine_message_location_list* messages, xmachine_message_location_PBM* partition_matrix, float x, float y, float z){
	...
	glm::ivec3 agent_grid_cell = message_location_grid_position(position);
...
__global__ void GPUFLAME_outputdata(xmachine_memory_Boid_list* agents, xmachine_message_location_list* location_messages){
	...
	agent.velocity = agents->velocity[index];
	...
	agents->velocity[index] = agent.velocity;
}
