...
cudaGraphicsResource_t Boid_default_cgr;
...
__global__ void output_Boid_agent_to_VBO(xmachine_memory_Boid_list* agents, glm::vec4* vbo, glm::vec3 centralise){
    ...
    vbo[index].x = agents->position[index].x - centralise.x;
    vbo[index].y = agents->position[index].y - centralise.x;
    vbo[index].z = agents->position[index].z - centralise.x;
    vbo[index].w = 1.0;
}
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&Boid_default_cgr, &Boid_default_tbo, &Boid_default_displacementTex, xmachine_memory_Boid_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_Boid_default_count() > 0)
	{
        ...
        gpuErrchk(cudaGraphicsMapResources(1, &Boid_default_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, Boid_default_cgr));
		//cuda block size
		...
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &Boid_default_cgr));
	}
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case(27) :
		...
		deleteTBO( &Boid_default_cgr, &Boid_default_tbo);
