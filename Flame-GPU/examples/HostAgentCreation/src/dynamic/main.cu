...
void runConsoleWithXMLOutput(int iterations, int outputFrequency){
	...
	for (int i=0; i< iterations; i++)
	{
		...
		if((i+1) % outputFrequency == 0){
			saveIterationData(outputpath, i+1, get_host_Agent_default_agents(), get_device_Agent_default_agents(), get_agent_Agent_default_count(),get_host_Agent_s2_agents(), get_device_Agent_s2_agents(), get_agent_Agent_s2_count());
	...
	if(iterations % outputFrequency != 0){
		saveIterationData(outputpath, iterations, get_host_Agent_default_agents(), get_device_Agent_default_agents(), get_agent_Agent_default_count(),get_host_Agent_s2_agents(), get_device_Agent_s2_agents(), get_agent_Agent_s2_count());
		printf("Iteration %i Saved to XML\n", iterations);
	}
