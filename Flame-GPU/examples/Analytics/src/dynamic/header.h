 ...
 * @param agent Pointer to an agent structure of type xmachine_memory_Circle. This represents a single agent instance and can be modified directly.
 ...
 */
extern void saveIterationData(char* outputpath, int iteration_number, xmachine_memory_Circle_list* h_Circles_default, xmachine_memory_Circle_list* d_Circles_default, int h_xmachine_memory_Circle_default_count);
 ...
 * @param h_xmachine_memory_Circle_count Pointer to agent counter
 */
extern void readInitialStates(char* inputpath, xmachine_memory_Circle_list* h_Circles, int* h_xmachine_memory_Circle_count);
 ...
 */
extern int get_agent_Circle_MAX_count();
 ...
 */
extern int get_agent_Circle_default_count();

/** reset_default_count
 * Resets the agent count of the Circle in state default to 0. This is useful for interacting with some visualisations.
 */
 ...
 */
extern xmachine_memory_Circle_list* get_device_Circle_default_agents();
 ...
 */
extern xmachine_memory_Circle_list* get_host_Circle_default_agents();
...
/** h_allocate_agent_Circle
 * Utility function to allocate and initialise an agent struct on the host.
 * @return address of a host-allocated Circle struct.
 */
...
/** h_allocate_agent_Circle_array
 * Utility function to allocate an array of structs for  Circle agents.
...
/** h_add_agent_Circle_default
 * Host function to add a single agent of type Circle to the default state on the device.
 * This invokes many cudaMempcy, and an append kernel launch. 
 * If multiple agents are to be created in a single iteration, consider h_add_agent_Circle_default instead.
...
/** h_add_agents_Circle_default(
 * Host function to add multiple agents of type Circle to the default state on the device if possible.
 * This includes the transparent conversion from AoS to SoA, many calls to cudaMemcpy and an append kernel.
 * @param agents pointer to host struct of arrays of Circle agents
...
__constant__ float KR;

__constant__ float KA;

/** set_KR
 * Sets the constant variable KR on the device which can then be used in the agent functions.
 * @param h_KR value to set the variable
 */
extern void set_KR(float* h_KR);

extern const float* get_KR();


extern float h_env_KR;

/** set_KA
 * Sets the constant variable KA on the device which can then be used in the agent functions.
 * @param h_KA value to set the variable
 */
extern void set_KA(float* h_KA);

extern const float* get_KA();


extern float h_env_KA;
