...
#endif
    setInitialConstants();
    PROFILE_PUSH_RANGE("setInitialConstants");
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_INIT_FUNCTIONS) && INSTRUMENT_INIT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: setInitialConstants = %f (ms)\n", instrument_milliseconds);
#endif
...
PROFILE_SCOPED_RANGE("singleIteration");
  ...
  /* set all non partitioned, spatial partitioned and On-Graph Partitioned message counts to 0*/
	...
	cudaEventRecord(instrument_start);
#endif
    PROFILE_PUSH_RANGE("stepFunction");
	stepFunction();
	...
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: stepFunction = %f (ms)\n", instrument_milliseconds);
#endif
...
//constant setter
void set_KR(float* h_KR){
    gpuErrchk(cudaMemcpyToSymbol(KR, h_KR, sizeof(float)));
    memcpy(&h_env_KR, h_KR,sizeof(float));
}
...
//constant setter
void set_KA(float* h_KA){
    gpuErrchk(cudaMemcpyToSymbol(KA, h_KA, sizeof(float)));
    memcpy(&h_env_KA, h_KA,sizeof(float));
}
...
/** float get_Circle_default_variable_x(unsigned int index)
 ...
 */
__host__ float get_Circle_default_variable_x(unsigned int index){
    unsigned int count = get_agent_Circle_default_count();
    unsigned int currentIteration = getIterationNumber();
...
/** float get_Circle_default_variable_y(unsigned int index)
 ...
 */
__host__ float get_Circle_default_variable_y(unsigned int index){
    unsigned int count = get_agent_Circle_default_count();
    unsigned int currentIteration = getIterationNumber();
...
/** float get_Circle_default_variable_z(unsigned int index)
 ...
 */
__host__ float get_Circle_default_variable_z(unsigned int index){
    unsigned int count = get_agent_Circle_default_count();
    unsigned int currentIteration = getIterationNumber();
...
/** float get_Circle_default_variable_fx(unsigned int index)
 ...
 */
__host__ float get_Circle_default_variable_fx(unsigned int index){
    unsigned int count = get_agent_Circle_default_count();
    unsigned int currentIteration = getIterationNumber();
...
/** float get_Circle_default_variable_fy(unsigned int index)
 ...
 */
__host__ float get_Circle_default_variable_fy(unsigned int index){
    unsigned int count = get_agent_Circle_default_count();
    unsigned int currentIteration = getIterationNumber();
...
float reduce_Circle_default_x_variable(){
    //reduce in default stream
...
float min_Circle_default_x_variable(){
    //min in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->x);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float max_Circle_default_x_variable(){
    //max in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->x);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float reduce_Circle_default_y_variable(){
    //reduce in default stream
...
float min_Circle_default_y_variable(){
    //min in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->y);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float max_Circle_default_y_variable(){
    //max in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->y);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float reduce_Circle_default_z_variable(){
    //reduce in default stream
...
float min_Circle_default_z_variable(){
    //min in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->z);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float max_Circle_default_z_variable(){
    //max in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->z);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float reduce_Circle_default_fx_variable(){
    //reduce in default stream
...
float min_Circle_default_fx_variable(){
    //min in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->fx);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float max_Circle_default_fx_variable(){
    //max in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->fx);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float reduce_Circle_default_fy_variable(){
    //reduce in default stream
...
float min_Circle_default_fy_variable(){
    //min in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->fy);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
float max_Circle_default_fy_variable(){
    //max in default stream
    thrust::device_ptr<float> thrust_ptr = thrust::device_pointer_cast(d_Circles_default->fy);
    size_t result_offset = thrust::max_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Circle_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
