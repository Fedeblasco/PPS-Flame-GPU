...
/** crystal_simulate
 * Agent function prototype for simulate function of crystal agent
 */
void crystal_simulate(cudaStream_t &stream);
...
PROFILE_SCOPED_RANGE("singleIteration");
  ...
  /* set all non partitioned, spatial partitioned and On-Graph Partitioned message counts to 0*/
    ...
    PROFILE_PUSH_RANGE("crystal_simulate");
	crystal_simulate(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: crystal_simulate = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
...
//constant setter
void set_I_agg(float* h_I_agg){
    gpuErrchk(cudaMemcpyToSymbol(I_agg, h_I_agg, sizeof(float)));
    memcpy(&h_env_I_agg, h_I_agg,sizeof(float));
}

//constant getter
const float* get_I_agg(){
    return &h_env_I_agg;
}
...
//constant setter
void set_dT(float* h_dT){
    gpuErrchk(cudaMemcpyToSymbol(dT, h_dT, sizeof(float)));
    memcpy(&h_env_dT, h_dT,sizeof(float));
}
...
//constant setter
void set_aggNo(int* h_aggNo){
    gpuErrchk(cudaMemcpyToSymbol(aggNo, h_aggNo, sizeof(int)));
    memcpy(&h_env_aggNo, h_aggNo,sizeof(int));
}
...
/* Shared memory size calculator for agent function */
int crystal_simulate_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
...
void crystal_simulate(cudaStream_t &stream){
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_simulate, crystal_simulate_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = crystal_simulate_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (simulate)
	//Reallocate   : true
	//Input        : internal_coord
	//Output       : 
	//Agent Output : 
	GPUFLAME_simulate<<<g, b, sm_size, stream>>>(d_crystals, d_internal_coords);
	gpuErrchkLaunch();
	...
	xmachine_memory_crystal_list* simulate_crystals_temp = d_crystals;
	d_crystals = d_crystals_swap;
	d_crystals_swap = simulate_crystals_temp;
	//reset agent count
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_crystal_default_count+h_xmachine_memory_crystal_count > xmachine_memory_crystal_MAX){
		printf("Error: Buffer size of simulate agents in state default will be exceeded moving working agents to next state in function simulate\n");
      exit(EXIT_FAILURE);
      }
