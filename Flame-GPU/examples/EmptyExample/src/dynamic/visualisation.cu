...
cudaGraphicsResource_t example_agent_default_cgr;
GLuint example_agent_default_tbo;
GLuint example_agent_default_displacementTex;
...
__global__ void output_example_agent_agent_to_VBO(xmachine_memory_example_agent_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&example_agent_default_cgr, &example_agent_default_tbo, &example_agent_default_displacementTex, xmachine_memory_example_agent_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_example_agent_default_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &example_agent_default_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, example_agent_default_cgr));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_example_agent_default_count()/threads_per_tile);
		...
		output_example_agent_agent_to_VBO<<< grid, threads>>>(get_device_example_agent_default_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &example_agent_default_cgr));
	}
...
void display()
{
	...
	//Draw example_agent Agents in default state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, example_agent_default_displacementTex);
	//loop
	for (int i=0; i< get_agent_example_agent_default_count(); i++){
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case(27) :
		...
		deleteTBO( &example_agent_default_cgr, &example_agent_default_tbo);
