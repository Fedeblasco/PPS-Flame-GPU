...
__constant__ int d_xmachine_memory_example_agent_count;

/* Agent state count constants */

__constant__ int d_xmachine_memory_example_agent_default_count;
...
__constant__ int d_message_example_message_count;         /**< message list counter*/
__constant__ int d_message_example_message_output_type;   /**< message output type (single or optional)*/
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created example_agent agent functions */

/** reset_example_agent_scan_input
 * example_agent agent reset scan input function
 * @param agents The xmachine_memory_example_agent_list agent list
 */
__global__ void reset_example_agent_scan_input(xmachine_memory_example_agent_list* agents){
...
/** scatter_example_agent_Agents
 * example_agent scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_example_agent_list agent list destination
 * @param agents_src xmachine_memory_example_agent_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void scatter_example_agent_Agents(xmachine_memory_example_agent_list* agents_dst, xmachine_memory_example_agent_list* agents_src, int dst_agent_count, int number_to_scatter){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** append_example_agent_Agents
 * example_agent scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_example_agent_list agent list destination
 * @param agents_src xmachine_memory_example_agent_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void append_example_agent_Agents(xmachine_memory_example_agent_list* agents_dst, xmachine_memory_example_agent_list* agents_src, int dst_agent_count, int number_to_append){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
 ...
 * Continuous example_agent agent add agent function writes agent data to agent swap
 * @param agents xmachine_memory_example_agent_list to add agents to 
...
__device__ void add_example_agent_agent(xmachine_memory_example_agent_list* agents, int id, float x, float y, float colour){
...
__device__ void add_example_agent_agent(xmachine_memory_example_agent_list* agents, int id, float x, float y, float colour){
    add_example_agent_agent<DISCRETE_2D>(agents, id, x, y, colour);
}

/** reorder_example_agent_agents
 * Continuous example_agent agent areorder function used after key value pairs have been sorted
 ...
 */
__global__ void reorder_example_agent_agents(unsigned int* values, xmachine_memory_example_agent_list* unordered_agents, xmachine_memory_example_agent_list* ordered_agents)
{
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created example_message message functions */


/** add_example_message_message
 * Add non partitioned or spatially partitioned example_message message
 * @param messages xmachine_message_example_message_list message list to add too
 * @param x agent variable of type float
 * @param y agent variable of type float
 */
__device__ void add_example_message_message(xmachine_message_example_message_list* messages, float x, float y){

	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x + d_message_example_message_count;

	int _position;
	int _scan_input;

	//decide output position
	if(d_message_example_message_output_type == single_message){
		_position = index; //same as agent position
		_scan_input = 0;
	}else if (d_message_example_message_output_type == optional_message){
		_position = 0;	   //to be calculated using Prefix sum
		_scan_input = 1;
	}

	//AoS - xmachine_message_example_message Coalesced memory write
...
/**
 * Scatter non partitioned or spatially partitioned example_message message (for optional messages)
 * @param messages scatter_optional_example_message_messages Sparse xmachine_message_example_message_list message list
 * @param message_swap temp xmachine_message_example_message_list message list to scatter sparse messages to
 */
__global__ void scatter_optional_example_message_messages(xmachine_message_example_message_list* messages, xmachine_message_example_message_list* messages_swap){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
	...
	//if optional message is to be written
	if (_scan_input == 1){
		int output_index = messages_swap->_position[index] + d_message_example_message_count;

		//AoS - xmachine_message_example_message Un-Coalesced scattered memory write
...
/** reset_example_message_swaps
 * Reset non partitioned or spatially partitioned example_message message swaps (for scattering optional messages)
 * @param message_swap message list to reset _position and _scan_input values back to 0
 */
__global__ void reset_example_message_swaps(xmachine_message_example_message_list* messages_swap){
...
__device__ xmachine_message_example_message* get_first_example_message_message(xmachine_message_example_message_list* messages){

	extern __shared__ int sm_data [];
	char* message_share = (char*)&sm_data[0];
	
	//wrap size is the number of tiles required to load all messages
	int wrap_size = (ceil((float)d_message_example_message_count/ blockDim.x)* blockDim.x);
	...
	//SoA to AoS - xmachine_message_example_message Coalesced memory read
	xmachine_message_example_message temp_message;
	...
	int message_index = SHARE_INDEX(threadIdx.y*blockDim.x+threadIdx.x, sizeof(xmachine_message_example_message));
	xmachine_message_example_message* sm_message = ((xmachine_message_example_message*)&message_share[message_index]);
	sm_message[0] = temp_message;

	__syncthreads();

  //HACK FOR 64 bit addressing issue in sm
	return ((xmachine_message_example_message*)&message_share[d_SM_START]);
}

__device__ xmachine_message_example_message* get_next_example_message_message(xmachine_message_example_message* message, xmachine_message_example_message_list* messages){
	...
	if (i >= d_message_example_message_count)
		i = 0;
	...
	int message_index = SHARE_INDEX(i, sizeof(xmachine_message_example_message));
	return ((xmachine_message_example_message*)&message_share[message_index]);
}
 ...
 *
 */
__global__ void GPUFLAME_output_example(xmachine_memory_example_agent_list* agents, xmachine_message_example_message_list* example_message_messages){
	
	//continuous agent: index is agent position in 1D agent list
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
  
    //For agents not using non partitioned message input check the agent bounds
    if (index >= d_xmachine_memory_example_agent_count)
        return;
    

	//SoA to AoS - xmachine_memory_output_example Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_example_agent agent;
	...
	//FLAME function call
	int dead = !output_example(&agent, example_message_messages	);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_output_example Coalesced memory write (ignore arrays)
 ...
 *
 */
__global__ void GPUFLAME_input_example(xmachine_memory_example_agent_list* agents, xmachine_message_example_message_list* example_message_messages){
	...
	//SoA to AoS - xmachine_memory_input_example Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_example_agent agent;
	...
	//FLAME function call
	int dead = !input_example(&agent, example_message_messages);
	...
	//AoS to SoA - xmachine_memory_input_example Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->colour[index] = agent.colour;
