...
void saveIterationData(char* outputpath, int iteration_number, xmachine_memory_Man_list* h_Mans_unengaged, xmachine_memory_Man_list* d_Mans_unengaged, int h_xmachine_memory_Man_unengaged_count,xmachine_memory_Man_list* h_Mans_engaged, xmachine_memory_Man_list* d_Mans_engaged, int h_xmachine_memory_Man_engaged_count,xmachine_memory_Woman_list* h_Womans_default, xmachine_memory_Woman_list* d_Womans_default, int h_xmachine_memory_Woman_default_count)
{
	...
	for (int i=0; i<h_xmachine_memory_Man_unengaged_count; i++){
		...
		fputs("<id>", file);
        sprintf(data, "%d", h_Mans_unengaged->id[i]);
		fputs(data, file);
		fputs("</id>\n", file);
        
		fputs("<round>", file);
        sprintf(data, "%d", h_Mans_unengaged->round[i]);
		fputs(data, file);
		fputs("</round>\n", file);
        
		fputs("<engaged_to>", file);
        sprintf(data, "%d", h_Mans_unengaged->engaged_to[i]);
		fputs(data, file);
		fputs("</engaged_to>\n", file);
        
		fputs("<preferred_woman>", file);
        for (int j=0;j<1024;j++){
            fprintf(file, "%d", h_Mans_unengaged->preferred_woman[(j*xmachine_memory_Man_MAX)+i]);
            if(j!=(1024-1))
                fprintf(file, ",");
        }
	...
	for (int i=0; i<h_xmachine_memory_Man_engaged_count; i++){
		...
		fputs("<id>", file);
        sprintf(data, "%d", h_Mans_engaged->id[i]);
		fputs(data, file);
		fputs("</id>\n", file);
        
		fputs("<round>", file);
        sprintf(data, "%d", h_Mans_engaged->round[i]);
		fputs(data, file);
		fputs("</round>\n", file);
        
		fputs("<engaged_to>", file);
        sprintf(data, "%d", h_Mans_engaged->engaged_to[i]);
		fputs(data, file);
		fputs("</engaged_to>\n", file);
        
		fputs("<preferred_woman>", file);
        for (int j=0;j<1024;j++){
            fprintf(file, "%d", h_Mans_engaged->preferred_woman[(j*xmachine_memory_Man_MAX)+i]);
            if(j!=(1024-1))
                fprintf(file, ",");
        }
		fputs("</preferred_woman>\n", file);
	...
	for (int i=0; i<h_xmachine_memory_Woman_default_count; i++){
		...
		fputs("<id>", file);
        sprintf(data, "%d", h_Womans_default->id[i]);
		fputs(data, file);
		fputs("</id>\n", file);
        
		fputs("<current_suitor>", file);
        sprintf(data, "%d", h_Womans_default->current_suitor[i]);
		fputs(data, file);
		fputs("</current_suitor>\n", file);
        
		fputs("<current_suitor_rank>", file);
        sprintf(data, "%d", h_Womans_default->current_suitor_rank[i]);
		fputs(data, file);
		fputs("</current_suitor_rank>\n", file);
        
		fputs("<preferred_man>", file);
        for (int j=0;j<1024;j++){
            fprintf(file, "%d", h_Womans_default->preferred_man[(j*xmachine_memory_Woman_MAX)+i]);
            if(j!=(1024-1))
                fprintf(file, ",");
        }
		fputs("</preferred_man>\n", file);
	...
	*h_xmachine_memory_Woman_count = 0;
	...
	while(reading==1)
	{
		...
		else if(c == '<')
		{
			...
			{
				if(in_Man_id){
                    Man_id = (int) fpgu_strtol(buffer); 
                }
				if(in_Man_round){
                    Man_round = (int) fpgu_strtol(buffer); 
                }
				if(in_Man_engaged_to){
                    Man_engaged_to = (int) fpgu_strtol(buffer); 
                }
				if(in_Man_preferred_woman){
                    readArrayInput<int>(&fpgu_strtol, buffer, Man_preferred_woman, 1024);    
                }
				if(in_Woman_id){
                    Woman_id = (int) fpgu_strtol(buffer); 
                }
				if(in_Woman_current_suitor){
                    Woman_current_suitor = (int) fpgu_strtol(buffer); 
                }
				if(in_Woman_current_suitor_rank){
                    Woman_current_suitor_rank = (int) fpgu_strtol(buffer); 
                }
				if(in_Woman_preferred_man){
                    readArrayInput<int>(&fpgu_strtol, buffer, Woman_preferred_man, 1024);    
                }
