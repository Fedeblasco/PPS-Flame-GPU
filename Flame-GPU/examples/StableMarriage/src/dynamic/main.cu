...
void runConsoleWithXMLOutput(int iterations, int outputFrequency){
	...
	for (int i=0; i< iterations; i++)
	{
		...
		if((i+1) % outputFrequency == 0){
			saveIterationData(outputpath, i+1, get_host_Man_unengaged_agents(), get_device_Man_unengaged_agents(), get_agent_Man_unengaged_count(),get_host_Man_engaged_agents(), get_device_Man_engaged_agents(), get_agent_Man_engaged_count(),get_host_Woman_default_agents(), get_device_Woman_default_agents(), get_agent_Woman_default_count());
	...
	if(iterations % outputFrequency != 0){
		saveIterationData(outputpath, iterations, get_host_Man_unengaged_agents(), get_device_Man_unengaged_agents(), get_agent_Man_unengaged_count(),get_host_Man_engaged_agents(), get_device_Man_engaged_agents(), get_agent_Man_engaged_count(),get_host_Woman_default_agents(), get_device_Woman_default_agents(), get_agent_Woman_default_count());
		printf("Iteration %i Saved to XML\n", iterations);
	}
