...
void runConsoleWithXMLOutput(int iterations, int outputFrequency){
	...
	for (int i=0; i< iterations; i++)
	{
		...
		if((i+1) % outputFrequency == 0){
			saveIterationData(outputpath, i+1, get_host_agent_default_agents(), get_device_agent_default_agents(), get_agent_agent_default_count(),get_host_agent_portador_agents(), get_device_agent_portador_agents(), get_agent_agent_portador_count(),get_host_agent_enfermo_agents(), get_device_agent_enfermo_agents(), get_agent_agent_enfermo_count(),get_host_navmap_static_agents(), get_device_navmap_static_agents(), get_agent_navmap_static_count());
